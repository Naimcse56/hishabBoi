                        ...
                        <tbody>
                            @foreach ($transactions as $key => $transaction)
                                ...
                                @foreach ($transaction->filter(function ($tran) {
                                    ...
                                    @endphp
                                    <tr>
                                        <td class="pl-25"><a target="_blank" class="text-black" href="{{route('accountings.ledger_report_details_specific_filter',['start_date' => $dateFrom, 'end_date' => $dateTo, 'account_id' => $item['id'], 'work_order_id' => $work_order->id])}}">{{$item['name']}}</a></td>
                                        <td class="nowrap text-right" width="20%"><a target="_blank" class="text-black" href="{{route('accountings.ledger_report_details_specific_filter',['start_date' => $dateFrom, 'end_date' => $dateTo, 'account_id' => $item['id'], 'work_order_id' => $work_order->id, 'reciept_payment' => 'rcv_cash'])}}">{{number_format($item['rcv_balance'],2)}}</a></td>
                                        <td class="nowrap text-right" width="20%"><a target="_blank" class="text-black" href="{{route('accountings.ledger_report_details_specific_filter',['start_date' => $dateFrom, 'end_date' => $dateTo, 'account_id' => $item['id'], 'work_order_id' => $work_order->id, 'reciept_payment' => 'rcv_bank'])}}">{{number_format($item['rcv_bank_balance'],2)}}</a></td>
                                    </tr>
                                @endforeach
                        ...
                        <tbody>
                            @foreach ($transactions as $key => $transaction)
                                ...
                                @foreach ($transaction->filter(function ($tran) {
                                    ...
                                    <tr>
                                    <td class="pl-25"><a target="_blank" class="text-black" href="{{route('accountings.ledger_report_details_specific_filter',['start_date' => $dateFrom, 'end_date' => $dateTo, 'account_id' => $item['id'], 'work_order_id' => $work_order->id])}}">{{$item['name']}}</a></td>
                                    <td class="nowrap text-right" width="20%"><a target="_blank" class="text-black" href="{{route('accountings.ledger_report_details_specific_filter',['start_date' => $dateFrom, 'end_date' => $dateTo, 'account_id' => $item['id'], 'work_order_id' => $work_order->id, 'reciept_payment' => 'pay_cash'])}}">{{number_format($item['pay_balance'],2)}}</a></td>
                                    <td class="nowrap text-right" width="20%"><a target="_blank" class="text-black" href="{{route('accountings.ledger_report_details_specific_filter',['start_date' => $dateFrom, 'end_date' => $dateTo, 'account_id' => $item['id'], 'work_order_id' => $work_order->id, 'reciept_payment' => 'pay_bank'])}}">{{number_format($item['pay_bank_balance'],2)}}</a></td>
                                    </tr>
                                @endforeach
