...
class IncomeStatementController extends Controller
{
    public function income_statement(Request $request)
    {
        ...
        $ledgers = Ledger::with(['categories:id,parent_id,name,code,type,acc_type,view_in_bs,view_in_is,level'])->get(['id','name','code','parent_id','acc_type','view_in_bs','view_in_is','type','level']);
        ...
        $tr_data = array();
        $parent_data = array();
        $first_section = $this->getChildrenSummary($ledgers->where('id',app('account_configurations')['inc_st_first_section']), $tr_data, $start_date, $end_date, $sub_concern_id, $data['prve_date_from'], $data['prve_date_end'], app('account_configurations')['inc_st_fourth_section'], 'report');
        $data['first_section'] = collect($first_section);
        $second_section = $this->getChildrenSummary($ledgers->where('id',app('account_configurations')['inc_st_second_section']), $tr_data, $start_date, $end_date, $sub_concern_id, $data['prve_date_from'], $data['prve_date_end'], 0, 'report');
        $data['second_section'] = collect($second_section);
        $third_section = $this->getChildrenSummary($ledgers->where('id',app('account_configurations')['inc_st_third_section']), $tr_data, $start_date, $end_date, $sub_concern_id, $data['prve_date_from'], $data['prve_date_end'], 0, 'report');
        $data['third_section'] = collect($third_section);
        $fourth_section = $this->getChildrenSummary($ledgers->where('id',app('account_configurations')['inc_st_fourth_section']), $tr_data, $start_date, $end_date, $sub_concern_id, $data['prve_date_from'], $data['prve_date_end'], 0, 'report');
        $data['fourth_section'] = collect($fourth_section);
        $fifth_section = $this->getChildrenSummary($ledgers->where('id',app('account_configurations')['inc_st_fifth_section']), $tr_data, $start_date, $end_date, $sub_concern_id, $data['prve_date_from'], $data['prve_date_end'], 0, 'report');
        $data['fifth_section'] = collect($fifth_section);
        $tax_section = $this->getChildrenSummary($ledgers->where('id',app('account_configurations')['inc_st_tax_expenses_section']), $tr_data, $start_date, $end_date, $sub_concern_id, $data['prve_date_from'], $data['prve_date_end'], 0, 'report');
    ...
    protected function getChildrenSummary($accounts, $tr_data, $start_date, $end_date, $sub_concern_id, $prev_start_date, $prev_end_date, $restricted_account = 0, $report_type)
    {
        foreach ($accounts->whereNotIn('id',[$restricted_account]) as $key => $child) {
            ...
            $new_data['children_balance'] = count($child->categories) > 0 ? $this->getChildrenBalance($child->categories, $children_balance, $start_date, $end_date, $sub_concern_id, $prev_start_date, $prev_end_date, $report_type, $restricted_account) : 0;
            ...
            if (count($child->categories) > 0) {
                $tr_data = $this->getChildrenSummary($child->categories, $tr_data, $start_date, $end_date, $sub_concern_id, $prev_start_date, $prev_end_date, $restricted_account, $report_type);
            }
        }
        return $tr_data;
    }

    protected function getChildrenBalance($accounts, $children_balance, $start_date, $end_date, $sub_concern_id, $prev_start_date, $prev_end_date, $report_type, $restricted_account = 0)
    {
        foreach ($accounts->whereNotIn('id',[$restricted_account]) as $key => $child) {
            ...
            if (count($child->categories) > 0) {
                $children_balance = $this->getChildrenBalance($child->categories, $children_balance, $start_date, $end_date, $sub_concern_id, $prev_start_date, $prev_end_date, $report_type, $restricted_account);
            }          
        }
