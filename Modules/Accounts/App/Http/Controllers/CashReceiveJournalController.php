...
class CashReceiveJournalController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = $this->journalRepositoryInterface->listForDataTable(['rcv_cash'],'cash_rcv_multiple');
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){      
                        return view('accounts::cash_receive_multiple.components.action', compact('row'));
                    })
                    ->rawColumns(['action','status'])
                    ->make(true);
        }
        return view('accounts::cash_receive_multiple.index');
    }
    ...
    {
        return view('accounts::cash_receive_multiple.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            session()->put('voucher_id',route('multi-cash-receive.print',encrypt($item->id)));
    ...
    public function show($id)
    {
        try {
            ...
            return view('accounts::cash_receive_multiple.show_modal', $data);
        } catch (\Exception $e) {
    ...
    public function edit($id)
    {
        try {
            ...
            if ($data['journal']->is_approve != 1 && $data['journal']->is_work_order_based == 0) {
                return view('accounts::cash_receive_multiple.edit', $data);
            }
    ...
    public function print($id)
    {
        try {
            $data['journal'] = $this->journalRepositoryInterface->findById(decrypt($id));
            return view('accounts::cash_receive_multiple.receipt_print', $data);
        } catch (\Exception $e) {
            return response()->json(["message_error" => $e->getMessage()]);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            session()->put('voucher_id',route('multi-cash-receive.print',encrypt($item->id)));
            return redirect()->route('multi-cash-receive.index')->with('success', 'Updated Successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', $e->getMessage());
        }
