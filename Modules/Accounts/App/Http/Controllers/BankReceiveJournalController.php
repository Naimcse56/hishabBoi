...
class BankReceiveJournalController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = $this->journalRepositoryInterface->listForDataTable(['rcv_bank'],'bank_rcv_multiple');
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){      
                        return view('accounts::bank_receive_multiple.components.action', compact('row'));
                    })
                    ->rawColumns(['action','status'])
                    ->make(true);
        }
        return view('accounts::bank_receive_multiple.index');
    }
    ...
    {
        return view('accounts::bank_receive_multiple.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            session()->put('voucher_id',route('multi-bank-receive.print',encrypt($item->id)));
    ...
    public function show($id)
    {
        try {
            ...
            return view('accounts::bank_receive_multiple.show_modal', $data);
        } catch (\Exception $e) {
    ...
    public function edit($id)
    {
        try {
            ...
            if ($data['journal']->is_approve != 1 && $data['journal']->is_work_order_based == 0) {
                return view('accounts::bank_receive_multiple.edit', $data);
            }
    ...
    public function print($id)
    {
        try {
            $data['journal'] = $this->journalRepositoryInterface->findById(decrypt($id));
            return view('accounts::bank_receive_multiple.receipt_print', $data);
        } catch (\Exception $e) {
            return response()->json(["message_error" => $e->getMessage()]);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($request->credit_amount as $key => $credit_amount) {
            ...
            if ($credit_amount > 0 && $request->credit_account_id[$key] > 0 && $request->debit_account_id[$key] > 0) {
                $credit_amounts[] = $request->credit_amount[$key];
                $credit_account_id[] = isset($request->credit_narration[$key]) ? $request->credit_narration[$key] : null;
                $credit_partner_id[] = isset($request->credit_sub_account_id[$key]) ? $request->credit_sub_account_id[$key] : 0;
                $credit_work_order_id[] = isset($request->work_order_id[$key]) ? $request->work_order_id[$key] : 0;
                $credit_work_order_site_detail_id[] = isset($request->work_order_site_detail_id[$key]) ? $request->work_order_site_detail_id[$key] : 0;
        ...
        try {
            ...
            DB::commit();
            session()->put('voucher_id',route('multi-bank-receive.print',encrypt($item->id)));
            return redirect()->route('multi-bank-receive.index')->with('success', 'Updated Successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', $e->getMessage());
        }
